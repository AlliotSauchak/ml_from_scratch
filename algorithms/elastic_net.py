...
class ElasticNet:
    def __init__(self, lr=0.01, epochs=1000, l1_ratio=0.5, alpha=0.01):
        self.lr = lr
        self.epochs = epochs
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.epochs):
            ...
            mse_loss = np.mean((y - h) ** 2)
            l1_penalty = self.alpha * self.l1_ratio * np.sum(np.abs(self.weights_))
            l2_penalty = self.alpha * (1 - self.l1_ratio) * np.sum(self.weights_ ** 2)

            loss = mse_loss + l1_penalty + l2_penalty
            self.loss_history_.append(loss)

            grad_mse_w = (-2 * X.T @ (y - h)) / n

            grad_l1 = self.alpha * self.l1_ratio * np.sign(self.weights_)
            grad_l2 = 2 * self.alpha * (1 - self.l1_ratio) * self.weights_
